...
public class AgentpitOAuthService {
    ...
    @Value("${agentpit.oauth.authorization-url:}")
    ...
    @Value("${agentpit.oauth.callback-url:https://me.candaigo.com/api/auth/agentpit/callback}")
    ...
    public String buildAuthorizationUrl(String state, String verifier) {
        ...
        StringBuilder builder = new StringBuilder(authorizationUrl);
        builder.append(authorizationUrl.contains("?") ? "&" : "?")
    ...
    public Map<String, Object> handleCallback(String code, String verifier, String ip) {
        ...
        if (!StringUtils.hasText(accessToken)) {
            throw new BusinessException("AgentPit OAuth 未返回 access_token");
        }
    ...
    private void ensureCoreConfig() {
        if (!StringUtils.hasText(authorizationUrl)) {
            throw new BusinessException("未配置 AGENTPIT_OAUTH_AUTHORIZATION_URL");
        }
        if (!StringUtils.hasText(tokenUrl)) {
            throw new BusinessException("未配置 AGENTPIT_OAUTH_TOKEN_URL");
        }
        if (!StringUtils.hasText(clientId)) {
            throw new BusinessException("未配置 AGENTPIT_OAUTH_CLIENT_ID");
        }
        if (!StringUtils.hasText(clientSecret)) {
            throw new BusinessException("未配置 AGENTPIT_OAUTH_CLIENT_SECRET");
        }
    }
    ...
    private JsonNode exchangeAuthorizationCode(String code, String verifier) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new BusinessException("AgentPit token 交换失败: " + response.body());
            }
            return parseSuccessfulResponse(response.body(), "AgentPit token 浜ゆ崲");
        } catch (BusinessException ex) {
            throw ex;
        } catch (Exception ex) {
            log.error("AgentPit token exchange failed", ex);
            throw new BusinessException("AgentPit token 交换失败，请稍后重试");
        }
    }
    ...
    private JsonNode fetchProfilePayload(String accessToken, JsonNode tokenPayload) {
        ...
        try {
            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new BusinessException("AgentPit 用户信息获取失败: " + response.body());
            }
            return parseSuccessfulResponse(response.body(), "AgentPit 鐢ㄦ埛淇℃伅鑾峰彇");
        } catch (BusinessException ex) {
            throw ex;
        } catch (Exception ex) {
            log.error("AgentPit userinfo fetch failed", ex);
            throw new BusinessException("AgentPit 用户信息获取失败，请稍后重试");
        }
    }
    ...
    private AgentpitProfile mapProfile(JsonNode payload) {
        ...
        if (!StringUtils.hasText(usernameBase) && !StringUtils.hasText(nickname) && !StringUtils.hasText(email)) {
            throw new BusinessException("AgentPit 用户信息不完整，无法创建本地账号");
        }
    ...
    private User findOrCreateUser(AgentpitProfile profile) {
        ...
        newUser.setEmail(StringUtils.hasText(profile.email()) ? profile.email() : null);
        newUser.setNickname(StringUtils.hasText(profile.nickname()) ? profile.nickname() : username);
        newUser.setAvatar(profile.avatar());
        newUser.setStatus(1);

        int rows = userMapper.insert(newUser);
        if (rows <= 0 || newUser.getId() == null) {
            throw new BusinessException("创建 AgentPit 本地账号失败");
        }
    ...
    private JsonNode parseSuccessfulResponse(String body, String actionName) throws Exception {
        ...
        if (root == null || root.isNull()) {
            throw new BusinessException(actionName + "澶辫触锛岃繑鍥炰负绌?");
        }
